#!/bin/bash
# wrapper-notebooklm-upload.sh — 按分类上传到 NotebookLM（凌晨 02:00）
...
if [ "$AGE_HOURS" -gt 120 ]; then
    echo "⚠️ Session 已过期 (${AGE_HOURS}h)，需要重新登录"
    exit 0
fi
...
upload_category() {
    ...
    "$NOTEBOOKLM_VENV" "$SYNC_SCRIPT" \
        ...
        --category "$CAT_DIR" 2>&1 | grep -E '✅|❌|📭|📄|上传|⚠️|📊|📓|═══|✓|新 Notebook|UUID'
